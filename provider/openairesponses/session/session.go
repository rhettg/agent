...
func (s *SessionStore) SaveParams(p responses.ResponseNewParams) error {
	err := s.initPath()
	if err != nil {
...
func (s *SessionStore) SaveResponse(r *responses.Response) error {
	err := s.initPath()
	if err != nil {
		return fmt.Errorf("failed to initialize data directory: %w", err)
	}
...
func (s *SessionStore) Middleware(
	ctx context.Context, params responses.ResponseNewParams, next openairesponses.ResponsesCompletionFn,
